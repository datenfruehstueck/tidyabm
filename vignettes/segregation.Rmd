...
agent_a <- create_agent() %>%
  ...
  add_rule('move',
           ...
           .consequence = function(me, abm) {
             spot <- grid_get_free_neighboring_spots(me, 
                                                     abm,
                                                     which = neighborhood_definition) %>%
               slice_sample(n = 1)
             grid_move(me, 
...
And, of course, we can compile relevant data for any publication we're preparing through the [ODD protocol](https://doi.org/10.18564/jasss.4259):
```{r odd}
e %>% 
  odd()
```
